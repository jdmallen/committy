...
3. **Make environment variables persistent:**
   ...
   **macOS (Bash):**
   ...
   echo 'export AZURE_OPENAI_ENDPOINT="https://your-endpoint.openai.azure.com"' >> ~/.bash_profile
   ...
   **macOS (Zsh - default shell):**
   ...
   echo 'export AZURE_OPENAI_ENDPOINT="https://your-endpoint.openai.azure.com"' >> ~/.zshrc
...
3. **Make environment variables persistent:**
   ...
   [Environment]::SetEnvironmentVariable('AZURE_OPENAI_ENDPOINT', 'https://your-endpoint.openai.azure.com', 'User')
...
```bash
git diff --cached | committy --stdin --api-key "your_key" --endpoint "https://your-endpoint.openai.azure.com" --deployment "your-deployment"
```
...
committy
git diff --cached | committy --stdin
committy --clipboard
```
...
1. **"Committy not found in PATH"**
   ...
   - Try running `committy --help` to test
...
4. **Clipboard warnings**
   - Install clipboard tools on Linux: `sudo apt install xsel` or `sudo apt install xclip`
...
- **macOS**: Works with both Bash and Zsh shells